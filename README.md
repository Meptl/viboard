<p align="center">
  <a href="https://vibekanban.com">
    <picture>
      <source srcset="frontend/public/logo-dark.svg" media="(prefers-color-scheme: dark)">
      <source srcset="frontend/public/logo-light.svg" media="(prefers-color-scheme: light)">
      <img src="frontend/public/logo-light.svg" alt="Viboard Logo" width="360">
    </picture>
  </a>
</p>

![](frontend/public/vibe-kanban-screenshot-overview.png)

## Overview

Parallel AI agent management using a kanban board.

## Installation

Electron builds are available in the Releases page.

npm package doesn't exist. Clone the repo and run `npm run dev`.


## Building
```
npm install
npm run dev
```
or
```
pnpm run build:electron
```
