...
Make sure Vibe Kanban is already running locally before connecting your MCP client.
...
| Tool | Purpose | Required Parameters | Optional Parameters | Returns |
|------|---------|-------------------|-------------------|---------|
| `create_attempt` | Start working on a task with a coding agent | `task_id`<br/>`executor`<br/>`base_branch` | `variant` | Attempt ID and confirmation |
| `list_attempts` | List attempts for a task | `task_id` | None | List of attempts for the task |

### Supported Executors

When using `create_attempt`, the following executors are supported (case-insensitive, accepts hyphens or underscores):
...
4. **Start Task Execution**: Use `create_attempt` to programmatically begin work on tasks with specific coding agents
...
Your MCP client will use the `create_attempt` tool with parameters like:
```json
{
...
Each step uses the appropriate MCP tool (`list_projects`, `list_tasks`, `create_task`, `create_attempt`, `list_attempts`) to manage the complete workflow from planning to execution.
