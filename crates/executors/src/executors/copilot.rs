...
use ts_rs::TS;
use uuid::Uuid;
use workspace_utils::{msg_store::MsgStore, path::get_viboard_temp_dir};
...
impl Copilot {
    fn build_command_builder(&self, log_dir: &str) -> CommandBuilder {
        let mut builder = CommandBuilder::new("npx -y @github/copilot@0.0.403").params([
            "--no-color",
            "--log-level",
            "debug",
            "--log-dir",
...
impl Copilot {
    ...
    async fn create_temp_log_dir(current_dir: &Path) -> Result<PathBuf, ExecutorError> {
        let base_log_dir = get_viboard_temp_dir().join("copilot_logs");
